...
class MembreGroupeAdmin(admin.ModelAdmin):
    list_display = ['entreprise', 'groupe', 'statut', 'date_ajout']
    list_filter = ['statut', 'date_ajout']
    search_fields = ['entreprise__nom', 'groupe__nom']
    readonly_fields = ['date_ajout']
    
    fieldsets = (
        ('Relation', {
            'fields': ('entreprise', 'groupe', 'statut')
        }),
        ('Métadonnées', {
...
class ConversationDirecteAdmin(admin.ModelAdmin):
    list_display = ['entreprise1', 'entreprise2', 'date_creation', 'derniere_activite']
    list_filter = ['date_creation', 'derniere_activite']
    search_fields = ['entreprise1__nom', 'entreprise2__nom']
    readonly_fields = ['date_creation', 'derniere_activite']
    
    fieldsets = (
        ('Participants', {
            'fields': ('entreprise1', 'entreprise2')
        }),
        ('Métadonnées', {
